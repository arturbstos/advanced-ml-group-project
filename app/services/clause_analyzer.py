...
Rules:
  ...
  - Return risk="low", title="{low_label}" for clauses that are genuinely standard boilerplate with no meaningful negotiation upside. Examples that are almost always low: standard severability, mutual non-disparagement, symmetric waiver language, boilerplate force majeure, standard entire-agreement recitals, symmetric notice-delivery requirements, simple non-exclusive IP licenses limited to the contract term (no transfer, no sub-licensing), deemed-acceptance clauses that require concrete written defect notices within 30 days (these protect the freelancer), standard governing-law and jurisdiction clauses selecting German courts, and mutual confidentiality obligations limited to 1–3 years covering only marked or recognisably confidential information.
